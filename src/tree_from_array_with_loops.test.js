...
const tree_from_array_with_loops = require('./tree_from_array_with_loops');
...
describe('tree_from_array_with_loops', function () {
    it('should accept no arguments', function () {
        assert.deepEqual(tree_from_array_with_loops().roots, []);
    });
    ...
    it('should respect order of items #1', function () {
        ...
        const result = tree_print(tree_from_array_with_loops(items)).trim();
        const expected = `
├── a
│   ├── x
│   └── b
...
`.trim();
    ...
    it('should handle basic input №1', function () {
        ...
        assert.deepEqual(tree_from_array_with_loops(input).roots, [node1]);
    });
    ...
    it('should pass random test', function () {
        for (let i = 0, end = valid.length; i < end; ++i) {
            ...
            const t2 = roots_flatten(tree_from_array_with_loops(clone(valid[i])).roots).sort(fcmp_nodes_id);
    ...
    it('should throw "Circular dependency detected" #1', function () {
        ...
        // -> b
        //     c
        const nodes = [{id: 'c', parent_id: 'b'}, {id: 'b', parent_id: 'a'}, {id: 'a',parent_id: 'b'}];
        const perm = array_permutations(nodes);
        for (let i = 0, end = perm.length; i < end; ++i) {
            const {loops} = tree_from_array_with_loops(clone(perm[i]));
            const actual = loops.map(v => v.map(vv => vv.id).sort().join('->')).join('\n');
    ...
    it('should throw "Circular dependency detected" #2', function () {
        ...
        for (let i = 0, end = perm.length; i < end; ++i) {
            const {loops} = tree_from_array_with_loops(clone(perm[i]));
            const actual = loops.map(v => v.map(vv => vv.id).sort().join('->')).join('\n');
            assert.strictEqual(actual, 'c->d->e');
        }
    });
    it('should throw "Circular dependency detected" #3', function () {
        const nodes = [
            {id: 'a', parent_id: null},
            {id: 'b', parent_id: 'a'},
            {id: 'c', parent_id: 'x'},
        ...
        for (let i = 0, end = perm.length; i < end; ++i) {
            const {loops} = tree_from_array_with_loops(clone(perm[i]));
            const actual = loops.map(v => v.map(vv => vv.id).sort().join('->')).join('\n');
            assert.strictEqual(actual, 'c->d->e->x');
        }
    });
    it('should throw "All rows should have unique ids"', function () {
        assert.throws(() => tree_from_array_with_loops([{id: 1}, {id: 1}]), /All rows should have unique ids/);
    });
});
